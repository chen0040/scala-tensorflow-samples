...
object TensorUtils {
  def getImageTensor(image: BufferedImage, imgWidth: Int, imgHeight: Int): Tensor[java.lang.Float] = {
    val channels = 3
  ...
  @throws[IOException]
  def getImageTensor(inputStream: InputStream): Tensor[java.lang.Float] = {
    val img = ImageIO.read(inputStream)
    val imgWidth = img.getWidth
    val imgHeight = img.getHeight
    getImageTensor(img, imgWidth, imgHeight)
  }
}